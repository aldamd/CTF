- glibc `2.31` heap challenge that offers weird primitives for file struct (I think) and some realloc action, but ultimately gives a generous `UAF` and `12` chunks to allocate with of virtually arbitrary size
### solve
```python
#!/usr/bin/env python3
